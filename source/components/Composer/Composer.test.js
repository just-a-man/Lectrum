...
describe('composer component:', () => {
    ...
    test('avatar prop should be present as a source in "img" tag', () => {
        expect(result.find('img').html()).toEqual(`<img src="${props.avatar}">`);
    });

    test('currentUserFirstName prop should be present as a placeholder in "textarea" tag', () => {
        expect(result.find('textarea').prop('placeholder')).toEqual(`What\'s on your mind, ${
            props.currentUserFirstName
        }?`);
    });

    test('_updateComment should be called once value has changed in textarea', () => {
        expect(_updateCommentSpy).toHaveBeenCalledTimes(1);
    });

    test('_submitOnEnter should be called once user has clicked enter in textarea', () => {
